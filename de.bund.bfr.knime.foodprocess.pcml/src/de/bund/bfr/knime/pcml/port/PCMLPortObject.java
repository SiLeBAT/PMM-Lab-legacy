...
public class PCMLPortObject implements PortObject {
	...
	/** The PCML Document. */
	@Getter private PCMLDocument pcmlDoc;
	/** The port spec. */
	...
	@Getter private int outportIndex;
    ...
    void loadFrom(final InputStream is)
        ...
        ProcessNode[] processNodes = 
        	pcmlDoc.getPCML().getProcessChain().getProcessNodeArray();        
        if (processNodes.length > 0) {        	
        	...
        	if (!processNode.getType().equals(ProcessNodeType.CONTAMINATING)) {
            	...
            	for (int i = 0; i < pcmlMatrices.length; i++) {
            		...
            		pcmlMatrix.getMatrix(); // NameAndDatabaseId matrix = 
            		matrices.put(new Matrix(pcmlMatrix.getMatrix().getName(),
            				pcmlMatrix.getMatrix().isSetDbId() ? pcmlMatrix.getMatrix().getDbId() : -1),
        	...
        	if (!processNode.getType().equals(ProcessNodeType.ADMIXING)) {
            	...
            	if (outport.getAgentRecipe() != null) {
    	        	...
    	        	for (int i = 0; i < pcmlAgents.length; i++) {
    	        		...
    	        		agents.put(new Agent(pcmlAgent.getAgent().getName(),
    	        				pcmlAgent.getAgent().isSetDbId() ? pcmlAgent.getAgent().getDbId() : -1),
    	        				pcmlAgent.getQuantity());
    	        	}
            	}
        	}

        	pcmlSpec = new PCMLPortObjectSpec(matrices, agents, 
