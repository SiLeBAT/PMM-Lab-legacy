...
public class PCMLUtil {
	...
	public static Outport addOutport(final ProcessNode processNode, 
		...
		for (Entry<Agent, Double> agent : spec.getAgents().entrySet()) {
			...
			incredient.setQuantity(agent.getValue());
		}
		...
		if (null != spec.getTemperature()) {
			outport.setVolume(Double.toString(spec.getTemperature()));
		}
		if (null != spec.getPressure()) {
			outport.setVolume(Double.toString(spec.getPressure()));
		}
		if (null != spec.getPH_value()) {
			outport.setVolume(Double.toString(spec.getPH_value()));
		}
		if (null != spec.getAw_value()) {
			outport.setVolume(Double.toString(spec.getAw_value()));
		}
		return outport;
	}
