...
public class AddonC extends JPanel {
	...
	public double[] getVolumeDef() {
		...
		for (int i=0;i<l2.size();i++) {
			try {
				result[i] = l2.get(i).getText() == null ? Double.NaN : Double.parseDouble(l2.get(i).getText());
			}
			catch (Exception e) {}
		}
		return result;
	...
	private void buttonActionPerformed(ActionEvent e) {
		...
		MyTable myT = DBKernel.myList.getTable((forMatrices ? "Matrices" : "Agenzien"));
	...
	private void initComponents() {
		...
		//======== this ========
		setBorder(new TitledBorder((forMatrices ? "Matrix" : "Agent") + " Selection"));
