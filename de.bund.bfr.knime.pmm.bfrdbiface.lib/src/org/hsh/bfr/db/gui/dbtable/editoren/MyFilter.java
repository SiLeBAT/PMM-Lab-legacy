...
public class MyFilter implements Filter {
	...
	public MyFilter(MyDBTable myDBTable1, String findString, LinkedHashMap<Integer, Vector<String>> userIDs, JTextField textField1) {
    		 ...
    		 if (findString.trim().length() == 0) {
        		 ...
        		 if (userIDs != null) {
            		 for (Integer row : rows2check) {
         		  		 loopInternal(vv, row.intValue(), mnTable, foreignFields, tm, findStrings);
            		 }        		 
        		 }
        		 else {
        			 for (int row = 0; row < numRows; row++) {
         		  		 loopInternal(vv, row, mnTable, foreignFields, tm, findStrings);
        			 }
        		 }
     ...
     private boolean loopInternal(Vector<Integer> vv, int row, String[] mnTable, MyTable[] foreignFields, TableModel tm, String[] findStrings) {
		 String res = "";
