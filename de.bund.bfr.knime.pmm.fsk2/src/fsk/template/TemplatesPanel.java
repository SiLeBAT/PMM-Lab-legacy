...
/**
 * The Panel displayed in the templates tab of an R snippet dialog.
 *
 * @author Heiko Hofer.
 */
...
public class TemplatesPanel extends JPanel {
    ...
    public TemplatesPanel(final Set<Class<?>> set, final TemplateController controller) {
        ...
        m_categoriesListener = e -> {
            // listener only added to m_categories
        ...
        m_templates.addListSelectionListener(e -> {
        ...
        TemplateProvider.getDefault().addChangeListener(e -> {
            updateCategories();
            updateTemplatesList(TemplateProvider.ALL_CATEGORY);
        });
    ...
    private void onSelectionInTemplateList(final Object selected) {
        ...
        if (selected != null) {
            ...
            boolean removeable = TemplateProvider.getDefault().isRemoveable(template);
    ...
    private void updateTemplatesList(final String category) {
        ...
        TemplateProvider provider = TemplateProvider.getDefault();
        provider.getTemplates(m_metaCategories, category).forEach(model::addElement);

        m_templates.setSelectedValue(selected, true);
    }

    /**
     * Render the name of Java Snippet templates.
     */
    private static class TemplateListCellRenderer extends DefaultListCellRenderer {
        @Override
        public Component getListCellRendererComponent(final JList<?> list, final Object value, final int index, final
        boolean isSelected, final boolean cellHasFocus) {
            RSnippetTemplate m = (RSnippetTemplate) value;
            return super.getListCellRendererComponent(list, m.getName(), index, isSelected, cellHasFocus);
        }
    }
    ...
    private JPanel createTemplatesPanel() {
        ...
        m_applyTemplateButton.setEnabled(false);
        m_applyTemplateButton.addActionListener(e -> {
            RSnippetTemplate template = m_templates.getSelectedValue();
            if (template != null) {
        ...
        m_removeTemplateButton.setEnabled(false);
        m_removeTemplateButton.addActionListener(e -> {
            RSnippetTemplate template = m_templates.getSelectedValue();
            if (template != null) {
