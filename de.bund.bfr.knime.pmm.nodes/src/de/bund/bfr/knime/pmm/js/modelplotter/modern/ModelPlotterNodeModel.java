...
/**
 * Model Plotter node model. Reading all plotables functions of input table and preparing the first
 * plotable for JavaScript view.
 * 
...
public final class ModelPlotterNodeModel
  ...
  protected PortObject[] performExecuteCreatePortObjects(PortObject svgImageFromView,
    ...
    if (m_config.isHeadLess()) {
      ...
      // required to create the basic JavaScript environment which is not available to Nashorn.
      /*
       * some change are applied to the D3 and Function-plot library to be able to run in headless
       * mode(Nashorn) updating these files will break the headless generation of the Plot.
       */
      engine
          .eval("load('https://bugs.openjdk.java.net/secure/attachment/11893/env.nashorn.1.2.js')");
      ...
      context.setBindings(engine.getBindings(ScriptContext.ENGINE_SCOPE),
          ScriptContext.ENGINE_SCOPE);

      engine.eval(
          Files.newBufferedReader(Paths.get(URIUtil.toURI(pmmPlotter)), StandardCharsets.UTF_8),
          context);

      URL d3 = FileLocator.find(bundle,
          new Path("js-src/de/bund/bfr/knime/pmm/js/modelplotter/d3.js"), null);
      d3 = FileLocator.toFileURL(d3);
      engine.eval(Files.newBufferedReader(Paths.get(URIUtil.toURI(d3)), StandardCharsets.UTF_8),
          context);
      ...
      engine.eval(
          Files.newBufferedReader(Paths.get(URIUtil.toURI(functionPlot)), StandardCharsets.UTF_8),
          context);

      JSObject jSObject =
          (JSObject) context.getAttribute("pmm_plotter", ScriptContext.ENGINE_SCOPE);
    ...
    // adding xmlPrimer and svgPrimer to the generated SVG Text and output the imagePort
  ...
  private String getDbuuid(KnimeTuple tuple) {
    ...
    if (gid == null || gid.equals("?") || gid.isEmpty()) {
      ...
      if (tuple.getPmmXml(Model1Schema.ATT_MODELCATALOG) != null)
        seed = ((CatalogModelXml) tuple.getPmmXml(Model1Schema.ATT_MODELCATALOG).get(0))
            .getFormula().hashCode();
      else
        seed = tuple.hashCode();
  ...
  private List<JsM1DataSchema> codeM1DataSchema(List<KnimeTuple> tuples) {
    ...
    for (KnimeTuple tuple : tuples) {
      ...
      if (tuple.getInt(Model1Schema.ATT_DATABASEWRITABLE) == null) {
        ...
        schema.setDatabaseWritable(
            Model1Schema.WRITABLE == tuple.getInt(Model1Schema.ATT_DATABASEWRITABLE));
      }
  ...
  private List<JsM12DataSchema> codeM12DataSchema(List<KnimeTuple> tuples) {
    ...
    for (List<KnimeTuple> combinedTuple : tertiaryModelMap.values()) {
      ...
      if (firstTuple.getInt(Model1Schema.ATT_DATABASEWRITABLE) == null) {
        ...
        boolean dbWritable =
            Model1Schema.WRITABLE == firstTuple.getInt(Model1Schema.ATT_DATABASEWRITABLE);
      ...
      for (int j = 0; j < combinedTuple.size(); j++) {
        ...
        if (atuple.getInt(Model2Schema.ATT_DATABASEWRITABLE) == null) {
          ...
          boolean dbWritable =
              Model2Schema.WRITABLE == atuple.getInt(Model2Schema.ATT_DATABASEWRITABLE);
  ...
  private List<KnimeTuple> decodeM2Schemas(List<JsM2Schema> schemas) {
    ...
    for (JsM2Schema schema : schemas) {
      ...
      Arrays.stream(schema.getIndepList().getIndeps()).map(Indep::toIndepXml)
          .forEach(indepDoc::add);
      tuple.setValue(Model2Schema.ATT_INDEPENDENT, indepDoc);

      // Model literature
      PmmXmlDoc mLitDoc = new PmmXmlDoc();
      if (schema.getmLit().getLiterature() != null) {
        Arrays.stream(schema.getmLit().getLiterature()).map(Literature::toLiteratureItem)
            .forEach(mLitDoc::add);
      }
