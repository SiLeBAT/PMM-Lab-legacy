...
public class WriterUtils {
  ...
  public static PMFCoefficient paramXml2Coefficient(ParamXml paramXml) {
    ...
    String unit =
        (paramXml.getUnit() == null) ? "dimensionless" : PMFUtil.createId(paramXml.getUnit());
  ...
  public static Uncertainties estModel2Uncertainties(EstModelXml estModel) {
    ...
    Uncertainties uncertainties =
        SBMLFactory.createUncertainties(id, modelName, comment, r2, rms, sse, aic, bic, dof);
  ...
  public static PMFSpecies createSpecies(AgentXml agentXml, String unit, String compartmentId) {
    ...
    String speciesName =
        (String) DBKernel.getValue("Agenzien", "ID", agentXml.getId().toString(), "Agensname");
    String casNumber;
    if (speciesName == null) {
      ...
      casNumber =
          (String) DBKernel.getValue("Agenzien", "ID", agentXml.getId().toString(), "Cas_Nummer");
    }
  ...
  public static PMFUnitDefinition createUnitFromDB(String unit) throws XMLStreamException {
    ...
    if (mathml != null && !mathml.isEmpty()) {
      ...
      if (transformationIndex == -1) {
        ...
        mathml =
            mathml.substring(0, transformationIndex) + mathml.substring(endTransformationIndex);
      }
      ...
      for (int i = 0; i < numOfUnits; i++) {
        ...
        double multiplier =
            sbmlUnit.isSetMultiplier() ? sbmlUnit.getMultiplier() : PMFUnit.DEFAULT_MULTIPLIER;
        int scale = sbmlUnit.isSetScale() ? sbmlUnit.getScale() : PMFUnit.DEFAULT_SCALE;
        double exponent =
            sbmlUnit.isSetExponent() ? sbmlUnit.getExponent() : PMFUnit.DEFAULT_EXPONENT;
  ...
  public static ModelRule createM1Rule(CatalogModelXml catModel, String variable,
    ...
    ModelRule rule =
        new ModelRule(variable, formula, formulaName, modelClass, catModelId, references);
    return rule;
  }
  ...
  public static void write(List<KnimeTuple> tuples, boolean isPMFX, String dir, String mdName,
      Metadata metadata, boolean splitModels, String notes, ExecutionContext exec,
      ModelType modelType) throws Exception {
