...
/**
 * Basis of framework for executing SED-ML.
 * If the SED-ML document has errors, it is possible, but  unlikely that the execution of a SED-ML task will
 *   occur successfully. Therefore clients should call doc.validate() and check for errors before using this class. 
 * <p>
 *  To use this class clients should subclass and implement the  abstract methods:
 *  <ul>
 *  <li> IRawSedmlSimulationResults executeSimulation(String model, UniformTimeCourse simConfig);
 ...
 *    </code>
 *    The returned <code>Map</code> can be used within a {@link SedMLResultsProcesser2} object to obtain processed results.
 * @author radams
 *
 */
public abstract class AbstractSedmlExecutor{
	...
	/**
	 * 
	 * @param model A non-null {@link SedML} model
	 * @param output An {@link Output} which we want to reproduce.
	 * @throws IllegalArgumentException if <code>model == null</code> or <code>output == null</code>.
	 */
	public AbstractSedmlExecutor(SedML model,Output output) {
	   ...
	   /**
     * Alternative constructor when no outputs are specified and 
     * we want to run a single time-course and return all variables.
     * @param model A non-null {@link SedML} model
     * @param task An {@link Task} which we want to run.
     * @throws IllegalArgumentException if <code>model == null</code> or <code>output == null</code>.
     */
	...
	/**
	 * Adds an {@link IModelResolver}. Subclasses should add new mode resolvers to acquire models from their URIs
	 * @param resolver A non-null {@link IModelResolver}.
	 */
	...
	/**
	 * Subclasses can add a failure item if any sub-classed methods do not work( e.g, a simulation failed).
	 * @param statusItem A non-null {@link ExecutionStatusElement}.
	 */
	...
	public final Map<AbstractTask, IRawSedmlSimulationResults> runSimulations (){
		...
		for (AbstractTask t: tasksToExecute){
		   Model m = sedml.getModelWithId(t.getModelReference());
		   ...
		   System.err.println(changedModel);
		   if(changedModel==null){
			   addStatus(new ExecutionStatusElement(null, modelResolver.getMessage(),
						  ExecutionStatusType.ERROR));
		   }
		   
		   System.err.println("Ready to execute");
			 IRawSedmlSimulationResults results = executeSimulation(changedModel, (UniformTimeCourse)sedml.getSimulation(t.getSimulationReference()));
			 if(results==null){
				 addStatus(new ExecutionStatusElement(null,"Simulation failed during execution: " + t.getSimulationReference() +" with model: " + t.getModelReference(),
						  ExecutionStatusType.ERROR));
				// return res;
			 }
	 ...
	 * @return <code>true</code> if the simulation was successfully executed, <code>false</code> otherwise.
	 */
	 ...
	 * @return A possibly empty but non-null <code>List</code> of {@link ExecutionStatusElement} objects.
	 */
	...
	/**
	 * Implementors should indicate whether or not they support the value of the model's language attribute,
	 *  in terms of the ability to simulate such a model.
	 * @param language A SED-ML supported modelling language.
	...
	/**
	 * Subclasses should implement this method to indicate whether they can support a particular simulation type.
	 * @param sim
	 * @return
	 */
	...
	/**
	 * Implementing subclasses should run the simulation in this method. If the simulation fails then this should be logged by
	 *  using the method:
	 *  <pre>
	 *  protected final  void addStatus(ExecutionStatusElement statusItem)
	 *  </pre>
	 *   and returning <code>null</code>.
	 *   If execution is successful, then a non-null {@link IRawSedmlSimulationResults} object should be returned.
	 * @param model A String of the XML of the model to simulate.
	 * @param simConfig The simulation configuration
	 * @return An object which implements {@link IRawSedmlSimulationResults} which will then be processed.
	 */
	 ...
	 * 
	 * @param model
	 *            A non-null {@link SedML} description.
