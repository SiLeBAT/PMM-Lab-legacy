...
/**
 * Processes raw simulation results according to instructions specified in the {@link DataGenerator} elements
 *  specified in the output.<br/>
 ...
 *     SedML sedml;
      ...
      // we get a java.util.Map that maps a task to a time-series result using that task.
      Map<AbstractTask, IRawSedmlSimulationResults> rawResults = impl.runSimulations();
       if(!impl.isExecuted()) {
          // show failure & exit
        }
...
public class SedMLResultsProcesser2  {
	...
	public class ProcessReport {
		...
		/**
		 * Returns an unmodifiable <code>List</code> of messages. This list may be empty but will not be null.
	...
	/**
	 * Constructor for creating a SedMLResultsProcesser2 object. Clients should ensure that the <code>output</code> argument
	 *  represents an {@link Output}  object defined in the <code>sedml argument</code>.
	 * @param sedml A non-null SED-ML object
	 * @param output A non-null <code>Output</code> belonging to the <code>sedml</code> argument.
	 * @throws IllegalArgumentException if any argument is <code>null</code> or if the output does not belong
	 *     to the {@link SedML} object ( based on the output id ).
	 */
	...
	/**
	 * Processes a set of simulation results and processes them according to the output passed into the constructor.
	 * There are several reasons that processing may fail, for example:
	 * <ul>
	 *  <li> If cross-references between outputs, variables and data generators are not valid.
	 *  <li> If mappings between the data column headers in the raw result, and model element identifiers cannot be resolved.
	 *  <li> If the MathML for the data post-processing is invalid or outside the scope of SED-ML's MathML usage.
	 * </ul>
	 *  Failures such as these will be logged and can be accessed by a call to getProcessReport() after calling this method.
	 * @param results
		...
		for (String dgId : output.getAllDataGeneratorReferences()) {
			...
			for (Variable v : vars) {
				...
				if (v.isVariable()) {
					...
					modelID = variable2IDResolver.getIdFromXPathIdentifer(v.getTarget());
					String taskRef=v.getReference();
					...
					var2Result.put(v.getId(), res);
					var2Data.put(v.getId(), rawTask2Results.get(t));
					Var2Model.put(v.getId(), modelID);
				// it's a symbol 
				} else if (v.isSymbol() && v.getSymbol().equals(VariableSymbol.TIME)) {
				    timeID=v.getId();
					var2Data.put(v.getId(), rawTask2Results.values().iterator().next());
					Var2Model.put(v.getId(), v.getId());
			...
			// now parse maths, and replace raw simulation results with processed results.
			...
			for (ASTCi var: identifiers){
				if(var.isVector()){
					...
					int otherVarInx = coll.getColumnIndexFor(Var2Model
							.get(varName));
					if (otherVarInx < 0
							|| otherVarInx >= var2Result.get(varName).getNumColumns()) {
					...
					for (ASTCi var : identifiers) {
						...
						if (!var.getName().equals(timeID)) {
							...
							otherVarInx = coll.getColumnIndexFor(Var2Model
									.get(var.getName()));
							if (otherVarInx < 0
									|| otherVarInx >= var2Result.get(var.getName()).getNumColumns()) {
								report.messages.add(new ExecutionStatusElement(null,NO_DATACOLuMN_FORID_MSG + var,ExecutionStatusType.ERROR));
								return ;
							}
						}
	...
	/**
	 * Optional method which can set a client implementation of an {@link IXPathToVariableIDResolver} interface.
	 * @param variable2idrEolver
	 */
    public void  setVariable2IDResolver(IXPathToVariableIDResolver variable2idrEolver) {
	...
	private IProcessedSedMLSimulationResults createData(List <double[]> processed, int NumRows) {

	
		String[] hdrs = new String[processed.size()];
		int colInd = 0;
		for(Iterator<String> it = output.getAllDataGeneratorReferences().iterator(); it.hasNext();){
			hdrs[colInd++] = it.next();
		}
	...
	/**
	 * Gets a processed result, if isProcessed() == <code>true</code>.
	 * The data encapsulated by this object is a copy of the data passed in. Further manipulation
	 * of the data backing this object will not alter the data in the original raw result.
