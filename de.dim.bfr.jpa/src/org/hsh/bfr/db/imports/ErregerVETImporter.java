 ...
 *
 */
public class ErregerVETImporter extends FileFilter implements MyImporter {
  /**
 ...
 */
	...
	public String getDescription() {
	    return "Erreger_VET Datei (*.xls; *.xlsx)";
	}
	...
	public void doImport(final String filename, final JProgressBar progress, final boolean showResults) {
      ...
      public void run() {
            ...
            progress.setString("Importiere Erreger_VET Datei...");
      			progress.setMinimum(0);
      			progress.setMaximum(650);
      			progress.setValue(0);
      		}
		      ...
		      sheet = wb.getSheet("VET_Erreger");      
		      PreparedStatement ps = DBKernel.getDBConnection().prepareStatement("INSERT INTO " + DBKernel.delimitL("Agenzien") +
		      		" (" + DBKernel.delimitL("VET_Code") + ", " + DBKernel.delimitL("Kurzbezeichnung") + ", " + DBKernel.delimitL("Agensname") + ") VALUES (?,?,?)");
		      ...
		      while( rows.hasNext() ) {
    			...
    			if (progress != null) {
    				...
    				if (myDB.getActualTable() != null) {
	    				...
	    				if (tablename.equals("Agenzien")) {
	    					myDB.setTable(myDB.getActualTable());
	    				}
    				}
    			}
  				else {
  					System.out.println("ErregerVETImporter - Fin");
  				}
		    }
