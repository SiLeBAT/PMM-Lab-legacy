...
public class MyDBTables {
	...
	public static void loadMyTables() {
		...
		addTable(DBKernel.users, MyList.SystemTabellen_LIST); // müsste jetzt doch gehen, oder?...  lieber die Users ganz weg, weil das Editieren auf dem HSQLServer nicht korrekt funktioniert - siehe im Trigger removeAccRight usw., da müsste man erst die sendRequests umstellen auf defaultconnection...		
		...
		MyTable newDoubleTable = new MyTable("DoubleKennzahlen",
				...
				"Verteilung der Werte bei Mehrfachmessungen, z.B. Normalverteilung. Anzugeben ist die entsprechende Funktion in R, z.B. rnorm(n, mean = 0, sd = 1)","geschätzt","'Parameter'/Zeit-Profil. Funktion des Parameters in Abhaengigkeit von der Zeit.\nFuer das Parsen wird die Klasse http://math.hws.edu/javamath/javadoc/edu/hws/jcm/data/Parser.html benutzt.","geschätzt","'Parameter'/x-Profil. Funktion des Parameters in Abhängigkeit des anzugebenden x-Parameters.\nFür das Parsen wird die Klasse http://math.hws.edu/javamath/javadoc/edu/hws/jcm/data/Parser.html benutzt.","Der zugehoerige x-Parameter: Bezugsgröße für eine Funktion, z.B. Temperatur in Abhaengigkeit von pH, dann ist die Bezugsgroesse pH.","geschätzt",
	  ...
	  hPM.put(new Integer(1), "+");	hPM.put(new Integer(2), "-");
	    ...
	    */
		MyTable tenazity_raw_data = new MyTable("Versuchsbedingungen", new String[]{"Referenz","Agens","AgensDetail","Matrix","EAN","MatrixDetail",
				...
				"Eindeutige ID aus der Combase Datenbank - bei eigenen Eintraegen bleibt das Feld leer","Eintrag aus der Combase Datenbank - bei eigenen Einträgen bleibt das Feld leer","Eintrag aus der Combase Datenbank - bei eigenen Einträgen bleibt das Feld leer","Eintrag aus der Combase Datenbank - bei eigenen Einträgen bleibt das Feld leer","Eintrag aus der Combase Datenbank - bei eigenen Einträgen bleibt das Feld leer"},
		...
		MyTable zutatendaten = new MyTable("Zutatendaten",
				...
				new String[]{"Verweis auf Eintraege aus der Tabelle Zutatendaten (Bedeutung nur für interne Verarbeitung)","Auswahl ob es sich um eine Zutat oder ein Produkt","Größe einer Charge","Mengengröße pro Chargenelement","Einheit eines Chargenelements","Produkt des Vorprozesses",
	...
	private static MyTable generateICD10Tabellen() {
		...
		MyTable ICD10_Kodes = new MyTable("ICD10_Kodes", new String[]{"Ebene","Ort","Art",
				...
				"IfSG-Meldung, kennzeichnet, dass bei Diagnosen,die mit dieser Schluesselnummer kodiert sind, besonders auf die Arzt-Meldepflicht nach dem Infektionsschutzgesetz IfSG) hinzuweisen ist: J = Ja; N = Nein","IfSG-Labor, kennzeichnet, dass bei Laboruntersuchungen zu diesen Diagnosen die Laborausschlussziffer des EBM (32006) gewählt werden kann: J = Ja; N = Nein"},
