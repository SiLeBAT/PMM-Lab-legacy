...
public class MyDBTablesNew extends MyDBI {
	...
	private void loadMyTables() {
		...
		addTable(us, SystemTabellen_LIST); // m�sste jetzt doch gehen, oder?...  lieber die Users ganz weg, weil das Editieren auf dem HSQLServer nicht korrekt funktioniert - siehe im Trigger removeAccRight usw., da m�sste man erst die sendRequests umstellen auf defaultconnection...		
		...
		MyTable literatur = new MyTable("Literatur", new String[]{"Erstautor","Jahr","Titel","Abstract","Journal","Volume","Issue","Seite","FreigabeModus","Webseite","Literaturtyp","Paper"},
				...
				new String[]{"Erstautor der Publikation","Ver�ffentlichungsjahr","Titel des Artikels","Abstract/Zusammenfassung des Artikels","Journal / Buchtitel / ggf. Webseite / Veranstaltung etc.",null,null,"Seitenzahl_Beginn","Auswahl ob diese Information oeffentlich zugaenglich sein soll: nie, nur in der Krise, immer - Auswahlbox",null,"Auswahl zwischen Paper, SOP, LA, Handbuch/Manual, Laborbuch","Originaldatei"},
		...
		MyTable newDoubleTable = new MyTable("DoubleKennzahlen",
				...
				"BOOLEAN","INTEGER"},
				new String[]{"Wert","Exponent zur Basis 10, falls vorhanden\nBeispiel 1.3*10^-4 : 1.3 wird in der Spalte 'Wert' eingetragen und -4 in dieser Spalte","Wert_typ ist entweder Einzelwert, Mittelwert oder Median","Der Einzelwert wurde nicht wirklich gemessen, sondern geschaetzt (ja=geschaetzt, nein=gemessen)","Anzahl der Wiederholungsmessungen/technischen Replikate f�r diesen Wert","geschaetzt","Standardabweichung des gemessenen Wertes - Eintrag nur bei Mehrfachmessungen m�glich","Exponent zur Basis 10 f�r die Standardabweichung, falls vorhanden","geschaetzt",
				"Minimum","Exponent zur Basis 10 f�r das Minimum, falls vorhanden","geschaetzt","Maximum","Exponent zur Basis 10 f�r das Maximum, falls vorhanden","geschaetzt",
				"Untere Konfidenzgrenze des gemessenen Wertes (95%-KI) - Eintrag nur bei Mehrfachmessungen m�glich","Exponent zur Basis 10 f�r LCL95, falls vorhanden","geschaetzt","Obere Konfidenzgrenze des gemessenen Wertes (95%-KI) - Eintrag nur bei Mehrfachmessungen m�glich","Exponent zur Basis 10 f�r UCL95, falls vorhanden","geschaetzt",
				"Verteilung der Werte bei Mehrfachmessungen, z.B. Normalverteilung. Anzugeben ist die entsprechende Funktion in R, z.B. rnorm(n, mean = 0, sd = 1)","geschaetzt","'Parameter'/Zeit-Profil. Funktion des Parameters in Abhaengigkeit von der Zeit.\nFuer das Parsen wird die Klasse http://math.hws.edu/javamath/javadoc/edu/hws/jcm/data/Parser.html benutzt.","geschaetzt","'Parameter'/x-Profil. Funktion des Parameters in Abh�ngigkeit des anzugebenden x-Parameters.\nF�r das Parsen wird die Klasse http://math.hws.edu/javamath/javadoc/edu/hws/jcm/data/Parser.html benutzt.","Der zugehoerige x-Parameter: Bezugsgroesse f�r eine Funktion, z.B. Temperatur in Abhaengigkeit von pH, dann ist die Bezugsgroesse pH.","geschaetzt",
		...
		MyTable diagnostik = new MyTable("Krankheitsbilder", new String[]{"Referenz","Agens","AgensDetail","Risikokategorie_CDC","BioStoffV",
				...
				"Letalitaetsdosis LD50: mittlere Dosis einer Substanz, die bei 50 % der Exponierten zum Tode f�hrt","Letalitaetsdosis50-Einheit","Bei welchem Organismus wurden die Untersuchungen zu LD50 gemacht?","Welche Aufnahmeroute wurde gewaehlt?",
				"Letalitaetsdosis LD100: mittlere Dosis einer Substanz, die bei 100 % der Exponierten zum Tode f�hrt","Letalitaetsdosis100-Einheit","Bei welchem Organismus wurden die Untersuchungen zu LD100 gemacht?","Welche Aufnahmeroute wurde gewaehlt?",
				...
				"Krankheit ist von Mensch zu Mensch �bertragbar","Therapiem�glichkeit besteht","Antidotgabe m�glich","Schutzimpfung verf�gbar","Moeglicher Zeitraum vom Symptombeginn bis zum Eintritt des Todes",
		...
		MyTable zertifikate = new MyTable("Zertifizierungssysteme", new String[]{"Bezeichnung","Abkuerzung","Anbieter"},
				...
				new String[]{"Vollstaendiger Name zum Zertifizierungssystem","Abk�rzung f�r Zertifizierungssystem","Anbieter des Zertifizierungssystems - Verweis auf die Kontakttabelle"},
		...
		MyTable labore = new MyTable("Labore", new String[]{"Kontakt","HIT_Nummer","ADV_Nummer",
				...
				"Ist das Labor privat, staatlich, oder sogar ein NRL (Nationales Referenz Labor) oder ein GPV (Gegenprobensachverstaendigen Labor) oder etwas anderes?","Matrices, auf die das Labor spezialisiert ist. Mehrfachnennungen m�glich.","Art der Untersuchung, Methoden. Mehrfachnennungen m�glich.","Agenzien, die das Labor untersucht und f�r die die genutzten Methodiken bekannt sind. Mehrfachnennungen m�glich."},
		...
		MyTable kits = new MyTable("Kits", new String[]{"Bezeichnung","Testanbieter","ZertifikatNr","Gueltigkeit","Zertifizierungssystem","AnbieterAngebot","Kosten","KostenEinheit",
				...
				"BOOLEAN","BOOLEAN",
				"VARCHAR(255)","VARCHAR(255)"},
				new String[]{"Bezeichnung des Kits","Verweis auf Eintrag in Kontakttabelle - falls Testanbieter vorhanden","Zertifikatnummer - falls vorhanden","G�ltigkeitsdatum des Zertifikats - falls vorhanden","Zertifizierungsanbieter - Verweis auf Tabelle Zertifizierungssysteme","Das Angebot kann ein individuelles Angebot, ein Katalogeintrag, eine E-Mail oder auch ein anderes Dokument des Testanbieters sein, moeglicherweise auch mit Angabe der Gueltigkeit des Angebots","Kosten f�r das Kit - Angabe ohne Mengenrabbatte. Geschaetzte Materialkosten, falls inhouse","Waehrung f�r die Kosten - Auswahlbox",
		...
		MyTable aufbereitungsverfahren = new MyTable("Aufbereitungsverfahren",
				...
				"Notwendige Kits - hier k�nnen mehrere Kits ausgewaehlt werden","Dauer des Verfahrens (von Beginn bis Vorliegen des Ergebnisses)","Zeiteinheit der Dauer","Wie gross ist der durchschnittliche TA-Zeitaufwand zur Durchfuehrung des Nachweisverfahrens - geschaetzt?","Zeiteinheit f�r Zeitaufwand f�r Personal - Auswahlbox",
				"Geschaetzte Materialkosten, zusaetzlich zu den Kitkosten","Waehrung f�r die Kosten - Auswahlbox",
		...
		MyTable labor_aufbereitungs_nachweisverfahren = new MyTable("Labor_Aufbereitungs_Nachweisverfahren",
				...
				new String[]{"Verweis zum Eintrag in Labor-Tabelle","Verweis zum Eintrag in Kombi-Tabelle Aufbereitungs_Nachweisverfahren","Zertifikatnummer - falls vorhanden","G�ltigkeitsdatum des Zertifikats - falls vorhanden","Zertifizierungsanbieter - Verweis auf Tabelle Zertifizierungssysteme","Angaben zum Durchsatz des Labors f�r das Verfahren - sollte im LaborAngebot angegeben sein","Einheit des Durchsatzes - Auswahlbox","Kosten pro Probe/Einzelansatz - ohne Rabatte - sollte im LaborAngebot angegeben sein","Waehrung f�r die Kosten - Auswahlbox","Auswahl ob diese Information oeffentlich zugaenglich sein soll: nie, nur in der Krise, immer - Auswahlbox","Nimmt das Labor auch externe Auftraege an?","Existiert eine SOP zu dem Verfahren bei dem Labor?","Das Angebot kann ein individuelles Angebot, ein Katalogeintrag, eine E-Mail oder auch ein anderes Dokument des Labors sein, moeglicherweise auch mit Angabe der Gueltigkeit des Angebots"},
	    ...
	    */
		...
		MyTable tenazity_measured_vals = new MyTable("Messwerte", new String[]{"Versuchsbedingungen","Zeit","ZeitEinheit",
				...
				"Falls angehakt:\nin den folgenden Feldern sind die Veraenderungen der Konzentration des Erregers im Vergleich zum Startzeitpunkt eingetragen.\nDabei bedeutet eine positive Zahl im Feld 'Konzentration' eine Konzentrationserh�hung, eine negative Zahl eine Konzentrationsreduzierung.","Konzentration des Erregers - Entweder ist die absolute Konzentration bzw. der Mittelwert bei Mehrfachmessungen hier einzutragen ODER die Konzentrationsaenderung, falls das Delta-Feld angehakt ist","Einheit zu den Konzentrationsangaben, auch der Logarithmus ist hier auswaehlbar - Auswahlbox",
		...
		MyTable prozessdaten = new MyTable("Prozessdaten",
				...
				"Fassungsverm�gen des Prozesselements, z.B. Volumen, Gewicht",null,"Bei einem kontinuierlichen Prozess muss die zeitliche Bezugsgroesse angegeben werden. Bei einem abgeschotteten Prozess bleibt das Feld leer",
		...
		MyTable zutatendaten = new MyTable("Zutatendaten",
				...
				new String[]{"Verweis auf Eintraege aus der Tabelle Zutatendaten (Bedeutung nur f�r interne Verarbeitung)","Auswahl ob es sich um eine Zutat oder ein Produkt","Groesse einer Charge","Mengengroesse pro Chargenelement","Einheit eines Chargenelements","Produkt des Vorprozesses",
	...
	private void doLieferkettenTabellen(final MyTable agenzien, final MyTable matrix, final LinkedHashMap<Object, String> h4) {
		...
		MyTable Knoten = new MyTable("Station", new String[]{"Produktkatalog","Name","Strasse","Hausnummer","Postfach","PLZ","Ort","District","Bundesland","Land","Longitude","Latitude","Ansprechpartner","Telefon","Fax","EMail","Webseite","Betriebsnummer","Betriebsart","VATnumber","Code",
				...
				"z.B. Endverbraucher, Erzeuger, Einzelh�ndler, Gro�h�ndler, Gastronomie, Mensch. Siehe weitere Beispiele ADV Katalog", null, "interner Code, z.B. NI00",
				"Falldefinition erf�llt (z.B. laut RKI) - Priorit�t: Wert zwischen 0 und 1",null,null,null,"Datum fr�hester Erkrankungsbeginn","Datum des H�hepunkt an Neuerkrankungen","Datum letzter Erkrankungsbeginn",null,null},
		...
		proce.put("erhitzt und nicht verzehrsf�hig (Vorprodukte wie eingefrorene Kuchen)", DBKernel.getLanguage().equalsIgnoreCase("en") ? "heated and not-ready-to-eat (e.g. frozen cake)" : "erhitzt und nicht verzehrsf�hig (Vorprodukte wie eingefrorene Kuchen)");
		proce.put("nicht erhitzt und nicht verzehrsf�hig (Rohwaren, die nicht zum Rohverzehr bestimmt sind wie Fleisch oder Eier)", DBKernel.getLanguage().equalsIgnoreCase("en") ? "not heated and not-ready-to-eat (meat, eggs)" : "nicht erhitzt und nicht verzehrsf�hig (Rohwaren, die nicht zum Rohverzehr bestimmt sind wie Fleisch oder Eier)");
		MyTable Produzent_Artikel = new MyTable("Produktkatalog", // Produzent_Artikel
				...
				new String[]{null,null,null,"gekocht? gesch�ttelt? ger�hrt?","wozu ist der Artikel gedacht? Was soll damit geschehen?","interner Code",null,null,null},
		...
		MyTable Lieferungen = new MyTable("Lieferungen", // Artikel_Lieferung
				new String[]{"Charge","dd_day","dd_month","dd_year","numPU","typePU", // "Artikel","ChargenNr","MHD",
					"Unitmenge","UnitEinheit","Empf�nger","Serial","EndChain","Explanation_EndChain","Contact_Questions_Remarks","Further_Traceback"}, // ,"Vorprodukt","Zielprodukt"
		...
		MyTable ChargenVerbindungen = new MyTable("ChargenVerbindungen",
				...
				new String[]{"Zutat.Empf�nger=Produkt.Artikel.Station", null, null});
	...
	private MyTable generateICD10Tabellen() {
		...
		MyTable ICD10_Kodes = new MyTable("ICD10_Kodes", new String[]{"Ebene","Ort","Art",
				...
				new String[]{"Klassifikationsebene, 1 Zeichen: 3 = Dreisteller; 4 = Viersteller; 5 = F�nfsteller","Ort der Schluesselnummer im Klassifikationsbaum, 1 Zeichen: T = terminale Schluesselnummer (kodierbarer Endpunkt); N = nichtterminale Schluesselnummer (kein kodierbarer Endpunkt)","Art der Vier- und F�nfsteller: X = explizit aufgef�hrt (praekombiniert); S = per Subklassifikation (postkombiniert)",
				...
				"untere Altersgrenze f�r eine Schluesselnummer: 999     = irrelevant; 000     = unter 1 vollendeten Tag; 001-006 = 1 Tag bis unter 7 Tage; 011-013 = 7 Tage bis unter 28 Tage; also 011 =  7-13 Tage (1 Woche bis unter 2 Wochen); 012 = 14-20 Tage (2 Wochen bis unter 3 Wochen); 013 = 21-27 Tage (3 Wochen bis unter einem Monat); 101-111 = 28 Tage bis unter 1 Jahr; also 101 = 28 Tage bis Ende des 2. Lebensmonats; 102 = Anfang bis Ende des 3. Lebensmonats; 103 = Anfang bis Ende des 4. Lebensmonats; usw. bis; 111 = Anfang des 12. Lebensmonats bis unter 1 Jahr; 201-299 = 1 Jahr bis unter 100 Jahre; 300-324 = 100 Jahre bis unter 125 Jahre",
				"untere Altersgrenze f�r eine Schluesselnummer, alternatives Format: 9999    = irrelevant; t000 - t365 = 0 Tage bis unter 1 Jahr; j001 - j124 = 1 Jahr bis unter 124 Jahre",
				"obere Altersgrenze f�r eine Schluesselnummer, wie bei Feld 'AltUnt'","obere Altersgrenze f�r eine Schluesselnummer,alternatives Format wie bei Feld 'AltUntNeu'",
				"Art des Fehlers bei Altersbezug: 9 = irrelevant; M = Muss-Fehler; K = Kann-Fehler","Krankheit in Mitteleuropa sehr selten? J = Ja; N = Nein","Schluesselnummer mit Inhalt belegt? J = Ja; N = Nein (--> Kann-Fehler ausl�sen!)",
	...
	private void generateStatUpModellTables(final MyTable literatur, final MyTable tenazity_raw_data, final LinkedHashMap<Object, String> hashZeit, final MyTable Konzentrationseinheiten, LinkedHashMap<Boolean, String> hYNB) {
		...
		MyTable GeschaetzteModelle = new MyTable("GeschaetzteModelle", new String[]{"Name","Versuchsbedingung","Modell",
				...
				"Referenzen, aus denen diese Modellschaetzung entnommen wurde","Verweis auf die Tabelle ModellkatalogParameter mit den geschaetzten Parametern","Verweis auf die Tabelle ModellkatalogParameterCovCor mit den Korrelationen der geschaetzten Parameter","G�ltigkeitsbereiche f�r Sekundaermodelle",null,null,null},
	...
	private void loadHashes() {		
		...
		hashBundesland.put("Baden-W�rttemberg", "Baden-W�rttemberg");
		...
		hashBundesland.put("Th�ringen", "Th�ringen");
