		...
		if (tablename.equals("Literatur")) {
			if (fieldname.equals("Jahr")) {
				...
				if (jahr < 1100 && jahr > diesjahr) {
					result = new String[1];
					result[0] = GuiMessages.getString("Merkw�rdige Jahresangabe...");
				}
			}
		}
		return result;
	...
	public static Vector<String[]> getPlausibilityRow(MyDBTable table, MyTable myT, int row, String idField) {
		...
		if (tablename.equals("Matrices")) {
			...
			if (table == null) {
				...
				result.add(new String[]{
						...
						GuiMessages.getString("Merkw�rdige Jahresangabe...")
						});
			}
		}
		...
		*/
			...
			result.add(new String[]{
					...
					"Jeder Messwert muss zu einer Versuchsbedingung geh�ren!"
					});
					*/
			result.add(new String[]{
					"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
					" WHERE " + DBKernel.delimitL("Delta") + " AND " + DBKernel.delimitL("Zeit") + " = 0",
					GuiMessages.getString("F�r den Zeitpunkt 0 kann das Delta Feld nicht angehakt sein!")
					});
			result.add(new String[]{
					"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
			...
			result.add(new String[]{
					...
					GuiMessages.getString("Die Einheit f�r die Konzentration sollte eingetragen werden!")
					});
			if (table == null) {
			}
			else if (row < 0) {
			...
			if (table == null) {
				result.add(new String[]{
						...
						GuiMessages.getString("Die Einheit f�r die Kosten sollte eingetragen werden!")
						});
				result.add(new String[]{
						"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
						...
						GuiMessages.getString("Wenn die ZertifikatNummer angegeben ist, dann sollte auch deren G�ltigkeit und das Zertifizierungssystem angegeben sein!")
						});
				result.add(new String[]{
						"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
			...
			if (table == null) {
				...
				result.add(new String[]{
						...
						GuiMessages.getString("Die Zeiteinheit f�r die Personalressourcen sollte eingetragen werden!")
						});
				result.add(new String[]{
						"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
			...
			if (table == null) {
				result.add(new String[]{
						...
						GuiMessages.getString("Die Einheit f�r die Nachweisgrenze sollte eingetragen werden!")
						});
				result.add(new String[]{
						"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
						" WHERE " + DBKernel.delimitL("Referenz") + " IS NULL",
			...
			if (table == null) {
				result.add(new String[]{
						...
						GuiMessages.getString("Die Einheit f�r den Durchsatz sollte eingetragen werden!")
						});
				result.add(new String[]{
						"SELECT " + idConf + " FROM " + DBKernel.delimitL(tablename) +
						" WHERE " + DBKernel.delimitL("Kosten") + " IS NULL OR " + DBKernel.delimitL("KostenEinheit") + " IS NULL",
