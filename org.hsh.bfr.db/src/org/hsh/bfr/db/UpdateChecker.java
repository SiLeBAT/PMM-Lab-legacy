...
// ACHTUNG: beim MERGEN sind sowohl KZ2NKZ als auch moveDblIntoDoubleKZ ohne Effekt!!! Da sie nicht im ChangeLog drin stehen!!!! Da muss KZ2NKZ nachtr�glich ausgef�hrt werden (solange die Tabelle Kennzahlen noch existiert). Bei moveDblIntoDoubleKZ???
...
public class UpdateChecker {
	...
	*/
		...
		if (isBios) {
			...
			DBKernel.sendRequest(
					"INSERT INTO " + DBKernel.delimitL("Einheiten") + " (" + DBKernel.delimitL("ID") + "," + DBKernel.delimitL("Einheit") + "," + DBKernel.delimitL("Beschreibung")
							+ ") VALUES (44,'�B�','Grad Baum�')", false); // 31 -> 44
			DBKernel.sendRequest(
					"INSERT INTO " + DBKernel.delimitL("Einheiten") + " (" + DBKernel.delimitL("ID") + "," + DBKernel.delimitL("Einheit") + "," + DBKernel.delimitL("Beschreibung")
	 ...
	 * SQLScriptImporter
	 * ().doImport("/org/hsh/bfr/db/res/002_EstModelSecView_165.sql", null,
	 * false); } public static void check4Updates_163_164() { /* // f�r Krisen
	 * EHEC-DB, da wurde wohl ein Update mitten "im �bergang" von
	 ...
	 * 
	 * ps.setString(1, "Frischgem�se"); ps.execute(); ps.setString(1,
	 * "Sprossgem�se"); ps.execute(); ps.setString(1, "Bockshornkleesprossen");
					...
					if (tn.equals(tableName)) {
						if (ct.equals("FOREIGN KEY") || ct.equals("UNIQUE")) {
							...
							if (!ct.equals("PRIMARY KEY") && !ct.equals("CHECK")) {
								MyLogger.handleMessage("Wasn das jetzt f�rn CONSTRAINT. Soll ich wirklich l�schen??? " + ct + "\t" + tn + "\t" + cn);
							}
						}
					}
				} while (rs.next());
