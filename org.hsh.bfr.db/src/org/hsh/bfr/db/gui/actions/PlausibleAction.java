		  		    ...
		  		    try {		  
		        		if (DBKernel.isKrise) {
			        		...
			        		if (progress != null) {
			        			...
			        			progress.setString("Plausibilit�tstests laufen...");
			        		...
			        		for(String key : myTables.keySet()) {
			        			...
			        			String tn = myT.getTablename();
			        	  		if (pd.radioButton1.isSelected()) { // Alle Datens�tze
			        	  		...
			        	  		/*
			        	  		if (analysedIDs != null && tn.equals("Versuchsbedingungen")) {// erstmal nur f�r Messwerte bzw. Versuchsbedingungen
	    	  						go4Table("Messwerte", result, -1, -1, DBKernel.myDBi.getTable("Messwerte"),
	    	  								null, showOnlyDataFromCurrentUser); // analysedIDs oder WHERE Versuchsbedingungen=5 oder so �hnlich vielleicht???
			        	  		}
			        	  		*/
						...
						if (rs != null && rs.first()) {
							do {
								...
								if (vals.length() > 0) {
									...
									result.add(tn + " hat " + rs.getString(1) + " Datens�tze mit denselben Werten: (" + vals + ")"); // Duplikate
								}
							} while (rs.next());
						}
					}
								    ...
								    for (int i=1;i<usernamen.length;i++) {
								    	if (!usernamen[i].equals(usernamen[0])) { // verschiedene Usernamen
								    		...
								    		if (rs3 != null && rs3.first()) {
								    			...
								    			// Datensatz bereits wieder gel�scht - System.err.print("\t" + rs2.getInt(4) + "\tnull"); 
								    		}
								    	}
								    }
						    	}
