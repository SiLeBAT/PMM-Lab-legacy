...
public class MyDBPanel extends JPanel {
  ...
  public boolean isMN() {
	...
	void button11ActionPerformed(ActionEvent e) {
		...
		if (parentDialog != null) {
			if (button10.isVisible()) {
				...
				savePressed = true; // wenn button10 nicht visible, dann haBEN WIR HIER EIN M:N Table, dann kann man ohnehin nicht abbrechen, dann wird die selectedID auch nicht ben�tigt! OK soll aber dennoch erhalten bleiben, um die Anwender nicht zu irritieren. Abbrechen ist mir zu bucklig zu implementieren!
			}
			else {
				  //if (myDBTable1.getActualTable().getTablename().equals("Messwerte")) { // erstmal nur f�r Messwerte, andere Tabellen k�nnen hinzugef�gt werden, muss aber erst gecheckt werden, vor allem wegen der Performance!!! Die scheint bei M:N Tabellen sehr  schlecht zu sein.
					  String toShow = "";
	...
	private void textField1FocusLost(FocusEvent e) {
		// nach filtern nimmt sich manchmal der JTable den Focus, das nervt, also dann bitte wieder zur�ck!
