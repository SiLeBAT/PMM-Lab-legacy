...
class MyDataChangeListener extends DatabaseChangeListener {
  ...
  public boolean beforeDelete(int row) {
  	if (table.getActualTable().isReadOnly() || (table.getMyDBPanel() != null && table.getMyDBPanel().isMN())) return false; // table.getMyDBPanel() == null bedeutet, dass es sich hier um myDBTable2 handelt... da soll schon gel�scht werden k�nnen
  	if (table.getActualTable().getTablename().equals("Users")) {
  		// Achtung: es sollte immer mindestens ein Admin vorhanden sein. Daher: L�schung nicht zulassen!!!
  		if (table.getValueAt(row, 1) != null && table.getValueAt(row, 1).toString().length() > 0) {
    		if (DBKernel.getUsername().equals(table.getValueAt(row, 1).toString())) {
  		    JOptionPane.showMessageDialog(table, "Aktiver User kann nicht gel�scht werden!", "L�schen nicht m�glich", JOptionPane.INFORMATION_MESSAGE);
  				return false;
    		}
    		/*
    		if (DBKernel.countUsers(true) == 1) {
    			...
    			if (oldAccRight == Users.ADMIN) {
    				JOptionPane.showMessageDialog(table, "Mindestens ein User muss Admin Rechte haben!", "L�schen nicht m�glich", JOptionPane.INFORMATION_MESSAGE);
    				return false;
    			}
    		}  	
    		*/		
    ...
    int retVal = JOptionPane.showConfirmDialog(table, "Sind Sie sicher, da� Sie die ausgew�hlte Zeile l�schen m�chten?", "L�schen best�tigen", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
