 ...
 */
				    ...
				    Hashtable<String, String> dbFieldNames = new Hashtable<String, String>();
				      ...
				      if (sql1.length() > 0 && sql2.length() > 0) {
					      String sql = "INSERT INTO " + DBKernel.delimitL(tableName) +
					      	" (" + sql1.substring(0, sql1.length() - 1) + ") VALUES (" + sql2.substring(0, sql2.length() - 1) + ")";
					      PreparedStatement ps = DBKernel.getDBConnection().prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
					      int idCol = lfdCol + 1;
					      sql = "UPDATE " + DBKernel.delimitL(tableName) + " SET " + sql3.substring(0, sql3.length() - 1) +
					      ...
					      while( rows.hasNext() ) {
					        ...
					        boolean setID = false;
			      			Integer lastID = null;
					        ps.clearParameters(); psUpdate.clearParameters();
						    for (codesI=0;codesI<codeSql1.size();codesI++) {psCodes[codesI].clearParameters(); doCode[codesI] = false;}
					      		...
					      		for (int j=0;j<numCols;j++) {
					      			if (fieldTypes[j] != null) {
					      				...
					      				*/
					      				...
					      				if (ffieldTypes[j].startsWith("VARCHAR(") || fieldTypes[j].startsWith("CHAR(") || fieldTypes[i].startsWith("BLOB(") && !tableName.equals("DateiSpeicher")) manageString(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j), hashBL);
					      				else if (ffieldTypes[j].equals("BOOLEAN")) manageBoolean(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j));
					      				else if (ffieldTypes[j].equals("INTEGER")) manageInteger(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j));
					      				else if (ffieldTypes[j].equals("BIGINT")) manageBigInteger(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j));
					      				else if (fieldTypes[j].equals("DATE")) manageDate(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j));
					      				else if (ffieldTypes[j].equals("DOUBLE")) {
					      					manageDouble(psForeign.get(myForeignTables[j]), psForeignUpdate.get(myForeignTables[j]), lfdColsForeign.get(myForeignTables[j]), row.getCell(j));
					      				}
							      	...
							      	if (lastID != null) {
									      ...
									      for (codesI=0;codesI<codeSql1.size();codesI++) {
									      	if (doCode[codesI]) {
										      	...
										      	try {psCodes[codesI].execute();numSuccess++;}
										      	catch (SQLException e1) {numFailed++;System.err.println(psCodes[codesI]);}
									      	}
									      }	
						      						...
						      						if (foreignTs[ii] != null && foreignTs[ii].equals(myT1)) {
						      							if (psForeign.get(myT1).executeUpdate() > 0) { // INSERT
												    		...
												    		// Das erstbeste Feld, das auf den Fremdtable verweist, wird mit dem Neueintrag verlinkt
						      								DBKernel.sendRequest("UPDATE " + DBKernel.delimitL(tableName) + " SET " + DBKernel.delimitL(myT.getFieldNames()[ii]) + "=" + lID + " WHERE " + DBKernel.delimitL("ID") + "=" + lastID, false);
						      							}
						      							break;
						      						}
						      					}
										    ...
										    /*
							      			for (int j=0;j<numCols;j++) {
							      				if (myForeignTables[j] != null && ffieldTypes[j] != null) {
							      					MyTable[] foreignTs = myT.getForeignFields(); 
							      					for (int ii=0;ii<foreignTs.length;ii++) {
							      						if (foreignTs[ii] != null && foreignTs[ii].equals(myForeignTables[j])) {
							      							if (psForeign.get(myForeignTables[j]).executeUpdate() > 0) { // INSERT
							      								int lID = DBKernel.getLastInsertedID(psForeign.get(myForeignTables[j]));
							      								DBKernel.sendRequest("UPDATE " + DBKernel.delimitL(tableName) + " SET " + DBKernel.delimitL(myT.getFieldNames()[ii]) + "=" + lID + " WHERE " + DBKernel.delimitL("ID") + "=" + lastID, false);
							      							}
							      							break;
							      						}
	...
	private String[] getTypes(String[] fieldNames, MyTable myT, String[] knownTypes) {
		...
		else result = knownTypes;
		for (int i=0;i<fieldNames.length;i++) {
			result[i] = getType(fieldNames[i], myT);
		}
		return result;
	}
	*/
	...
	private String getKZ(String xlsFieldName, String dbFieldName) {
    	if (xlsFieldName.equals(dbFieldName)) {
    		...
    		/*
    		if (xlsFieldName.startsWith(dbFieldName + "-Einzelwert")) return "Einzelwert";
    		if (xlsFieldName.startsWith(dbFieldName + "-Wiederholungen")) return "Wiederholungen";
    		if (xlsFieldName.startsWith(dbFieldName + "-Exponent")) return "Exponent";
    		if (xlsFieldName.startsWith(dbFieldName + "-Wert_typ")) return "Wert_typ";
    		if (xlsFieldName.startsWith(dbFieldName + "-Minimum")) return "Minimum";
    		if (xlsFieldName.startsWith(dbFieldName + "-Maximum")) return "Maximum";
    		if (xlsFieldName.startsWith(dbFieldName + "-Standardabweichung")) return "Standardabweichung";
    		if (xlsFieldName.startsWith(dbFieldName + "-LCL95")) return "LCL95";
    		if (xlsFieldName.startsWith(dbFieldName + "-UCL95")) return "UCL95";
    		if (xlsFieldName.startsWith(dbFieldName + "-Verteilung")) return "Verteilung";
    		if (xlsFieldName.startsWith(dbFieldName + "-Funktion (Zeit)")) return "Funktion (Zeit)";
    		if (xlsFieldName.startsWith(dbFieldName + "-Funktion (x)")) return "Funktion (x)";
    		if (xlsFieldName.startsWith(dbFieldName + "-x")) return "x";
    		if (xlsFieldName.startsWith(dbFieldName + "-Undefiniert (n.d.)")) return "Undefiniert (n.d.)";
    		*/
    	}	
    	return null;
	}
		...
		if (cell == null || cell.getCellType() == HSSFCell.CELL_TYPE_BLANK) {
			...
			if (hashBL != null) {
				...
				for (Object o : hashBL.keySet()) {
					...
					if (val < min) {min = val; newResult = o.toString();}
				}
