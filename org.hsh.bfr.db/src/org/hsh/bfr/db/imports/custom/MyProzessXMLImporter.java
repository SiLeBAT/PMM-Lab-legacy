...
public class MyProzessXMLImporter extends FileFilter implements MyImporter {
	...
	private String errorMessage = "";
			    ...
			    int numSuccess = 0;
	      		if (xmlFile.exists()) {
			    	...
			    	if (DBKernel.getUsername().equals("burchardi")) {
					    try {
					    	FileOutputStream fileOut = new FileOutputStream(filename + "_ife.xls"); // Der Export hier ist ja nur f�r IFE gedacht!
					    	wb.write(fileOut);
					    	fileOut.close();
					    }
					    catch (Exception e) {
		...
		private Vector<Integer> getReihenfolge(Vector<Integer[]> org_dst) {
			...
			// jetzt evtl. andere Enden �berpr�fen!
		...
		private int getReihenfolgePrev(Vector<Integer[]> org_dst, Vector<Integer> indexReihenfolge) {
			int wegPfeile = 0; // Ein Knoten kann mehrere Pfeile haben, die ihn verlassen ..., z.B. Salami_Test_Britta, gleich der erste Knoten "Wareneingang" f�hrt zum einen zu "Speck (Meat, Raw)" zum anderen zu "Schweinefleich , roh (Meat, Raw)"
			    ...
			    for (int i=0;i<rhflgn.size();i++) {
			    	...
			    	for (int j=0;j<rhflge.size();j++) {
			    		if (!alreadyAdded.containsKey(rhflge.get(j))) {
			    			...
			    			// wenn ich es nicht mache, dann klappen manche Imports von Wese nicht mehr, z.B. Rinderzunge, gep�kelt.PEX oder Vollmilch pasteurisiert standardisiert.PEX (Mails vom 20.03.2012)
			    			offset--;
			    		}
			    	}
			    }
