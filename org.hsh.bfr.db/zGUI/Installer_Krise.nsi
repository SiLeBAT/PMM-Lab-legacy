...
OutFile D:\workspaces\SiLeBAT_jars\krisenSetup_${VERSION}.exe
InstallDir $LOCALAPPDATA\Krisen-DB
CRCCheck on
XPStyle on
...
VIAddVersionKey /LANG=${LANG_GERMAN} ProductName Krisen-DB
...
Section -Main SEC0000
    ...
    Success:
        SetOutPath $INSTDIR
        SetOverwrite on
        File D:\workspaces\SiLeBAT_jars\Krisen_${VERSION}.jar
        File BfR_DB.ico
        SetOutPath $SMPROGRAMS\$StartMenuGroup
        CreateShortcut $SMPROGRAMS\$StartMenuGroup\Krisen-DB.lnk '"$JAVA_HOME\bin\javaw.exe"' '-Xms512m -Xmx768M -jar "$INSTDIR\Krisen_${VERSION}.jar"' "$INSTDIR\BfR_DB.ico" 0
        CreateShortCut $DESKTOP\Krisen-DB.lnk '"$JAVA_HOME\bin\javaw.exe"' '-Xms512m -Xmx768m -jar "$INSTDIR\Krisen_${VERSION}.jar"' "$INSTDIR\BfR_DB.ico" 0
...
Section /o -un.Main UNSEC0000
    Delete /REBOOTOK $DESKTOP\Krisen-DB.lnk
    Delete /REBOOTOK $SMPROGRAMS\$StartMenuGroup\Krisen-DB.lnk
    Delete /REBOOTOK $INSTDIR\Krisen_${VERSION}.jar
    Delete /REBOOTOK $INSTDIR\BfR_DB.ico
...
Function StartMenuGroupSelect
    Push $R1
    StartMenu::Select /autoadd /text "$(StartMenuPageText)" /lastused $StartMenuGroup Krisen-DB
    Pop $R1
